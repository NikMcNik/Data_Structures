...
//prints out matrix
void printMatrix (vector<vector<char> > &matrix, vector<vector<bool> > &Pmatrix){

    //set variables to change color of output
    ...
    for (int i = 0; i < matrix.size(); i++)
    {
        ...
        {   //check if position in matrix is true if so print out n red else in balck;
            if (Pmatrix[i][j]){
...
// goes throuhg positional matrix and update values
...
// starts search in each dirrection from position of first letter 
...
// find position of first letter of given word, if find the first letter starts search process
...
int main(int argc, char *argv[]){
    ...
    // for each given word calls search functions   
    for (int i = 1 ; i < argc; i++ ){
        findAndMark(letterMatrix,positionMatrix,argv[i] );
    }

    // prints out final matrix
    printMatrix(letterMatrix,positionMatrix);
